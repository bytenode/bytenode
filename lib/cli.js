...
if (program.flags.includes('--use')) {
  ...
  if (createLoaders && nextParam && !nextParam.startsWith('-')) {
    ...
    program.files[program.files.indexOf(nextParam)] = null;
  }

  program.files.filter(Boolean).forEach(function (filename) {
    ...
    try {
      ...
      if (createLoaders) {
        const loader = path.join(
          path.dirname(output),
          loaderPattern.replace('%',
            path.basename(output).replace(path.extname(output), '')));

        fs.writeFileSync(loader, loaderCode({ output }));
      }
    } catch (error) {
      console.error(error);
    }
  ...
  function loaderCode ({ output }) {
    return "'use strict';\n\n" +
      "require('bytenode');\n" +
      `require('./${path.relative(path.dirname(output), output)}');\n`;
  }
