...
const Bytenode = {
   ...
   * `Bytenode.run();` function will NOT return module.exports properly, and
   * if it is called twice, it will run the `bytecode` twice in the current
   * context, which will lead to issues and might crash the application.
  ...
  run ({ bytecode, filename }) {
    ...
    function _dummyCode (bytecode) {
      const sourceHash = _readSourceHash(bytecode);
    ...
    function _readSourceHash (bytecode) {
      if (process.version.startsWith('v8.8') ||
        process.version.startsWith('v8.9')) {
        return bytecode.subarray(12, 16).reduce((sum, number, power) => {
          sum += number * Math.pow(256, power);
          return sum;
        }, 0);
      } else {
        return bytecode.subarray(8, 12).reduce((sum, number, power) => {
          sum += number * Math.pow(256, power);
          return sum;
        }, 0);
      }
