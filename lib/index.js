...
class JavaScriptEntity {
  constructor (args) {
    assert(args && (args.filename || args.code),
      new Error('Either args.filename or args.code is required.'));

    assert(!args.filename || typeof args.filename === 'string',
      new TypeError('args.filename must be a string.'));

    assert(!args.code || typeof args.code === 'string',
      new TypeError('args.code must be a string.'));
