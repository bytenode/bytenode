...
const MAGIC_NUMBER = Buffer.from([0xc0, 0xde]);
const ZERO_LENGTH_EXTERNAL_REFERENCE_TABLE = Buffer.alloc(2);
const sheBangRegex = /^#!.*/;

function generateScript(bytecodeBuffer, options)
{
  if(!isBufferV8Bytecode(bytecodeBuffer))
  {
    // Try to decompress as Brotli
    bytecodeBuffer = brotliDecompressSync(bytecodeBuffer);

    ok(isBufferV8Bytecode(bytecodeBuffer), 'Invalid bytecode buffer');
  }
  ...
  const script = new vm.Script(dummyCode, options);
 ...
 */
const compileElectronCode = function (javascriptCode, compress) {
  return new Promise((resolve, reject) => {
    function onEnd()
    {
      if(compress) data = brotliCompressSync(data);
...
const compileFile = async function (args, output) {
  let filename, compileAsModule, compress, electron, createLoader, loaderFilename;
  ...
  if (electron) {
    bytecodeBuffer = await compileElectronCode(code, compress);
  } else {
